...
**`defaultValue` on select fields** — without a `defaultValue`, the select value starts as `undefined`, so `visible` conditions that depend on it won't evaluate correctly on first render. Always set a `defaultValue` that matches one of the options:
