...
**`Section` fills the boundary** — each step component receives section-level context via `useField`, checks `visible`, then uses `Section<T>` to render its own typed config fragment:
