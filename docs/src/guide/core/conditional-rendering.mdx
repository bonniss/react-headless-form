...
`disabled` does not unmount the field. The field stays registered and rendered, but RHF treats it as disabled.

In practice:

- the component still receives field props through `useField`
- the input should render as disabled in the UI
- disabled fields are excluded from the submit payload by RHF
